...
def calc_TEMPS_v045(modl,T0,Vox,dt,HT,CT,rho,k_param,cp,wType,w,Q,nFZ,tacq,Tb,BC,temp_file=None):
    ...
    # Calculate the maximum time step for stability of the thermal model
    w_max = np.max(w)               # Required parameters for max time step calculation
    rho_min = np.min(rho)           # Required parameters for max time step calculation
    cp_min = np.min(cp)             # Required parameters for max time step calculation
    k_max = np.max(k_param)               # Required parameters for max time step calculation
    dt_max = 1/(w_max/rho_min+2*k_max*(1+a**2+b**2)/(rho_min*cp_min*dx**2))  # Maximum allowable time step before iterations become unstable (s)
    if dt>dt_max:
